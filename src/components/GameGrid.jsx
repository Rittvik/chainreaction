            ...
            return (
              ...
              >
                {player && cell.count > 0 && (
                  player.emoji
                    ? /* ── emoji orb ── */
                      <div className={`orb-container orb-emoji-wrap orbs-${Math.min(cell.count, 3)}`}>
                        <span
                          className="orb-emoji"
                          style={{
                            color: player.color,
                        ...
                        {cell.count > 1 && (
                          <span className="orb-emoji-count" style={{ color: player.color }}>
                            ×{cell.count}
                          </span>
                        )}
                      </div>
                      ...
                      <div className={`orb-container orbs-${Math.min(cell.count, 3)}`}>
                        {Array.from({ length: Math.min(cell.count, 3) }, (_, i) => (
                          <div
                            key={i}
                            className="orb"
                            style={{ color: player.color, background: player.color, '--i': i }}
                          />
                        ))}
                      </div>
                )}
